...
- [ ] 1. Create label registry and fallback route utilities
  - [ ] 1.1 Create `app/utils/navigationLabels.ts` with `NavigationLabel` interface, `NAVIGATION_LABELS` array, `resolveLabel()` function, and `routePattern()` function
  ...
  - [ ] 1.2 Write property test for label resolution completeness
  ...
  - [ ] 1.3 Write property test for route pattern consistency
  ...
  - [ ] 1.4 Create `app/utils/navigationFallbacks.ts` with `FallbackMapping` interface, `FALLBACK_ROUTES` array, and `resolveFallbackRoute()` function
    - Implement the 8 fallback mappings from Requirement 5.2
    - Returns `"/"` for unmatched paths
    - _Requirements: 5.1, 5.2, 5.4_

  - [ ] 1.5 Write unit tests for label registry and fallback routes
...
- [ ] 3. Implement the `useNavigationStack` composable
  - [ ] 3.1 Create `app/composables/useNavigationStack.ts`
  ...
  - [ ] 3.2 Write property test for sessionStorage round-trip
  ...
  - [ ] 3.5 Write property test for back navigation with and without stack
  ...
  - [ ] 3.6 Write property test for invalid entry filtering
...
- [ ] 5. Create the global navigation middleware
  - [ ] 5.1 Create `app/middleware/stackTracker.global.ts`
  ...
  - [ ] 5.2 Write property test for query/hash-only navigation no-op
    - **Property 4: Query/Hash-Only Navigation Is a No-Op**
    - **Validates: Requirements 2.4**
    - Create `tests/properties/navStackQuerySkip.property.test.ts`

  - [ ] 5.3 Write property test for same-page-type replace vs different-page-type push
...
- [ ] 6. Migrate pages to use `useNavigationStack`
  - [ ] 6.1 Update `app/pages/parts/step/[stepId].vue` to use `useNavigationStack()`
  ...
  - [ ] 6.2 Update `app/pages/parts-browser/[id].vue` to use `useNavigationStack()`
  ...
  - [ ] 6.3 Update `app/pages/queue.vue` — remove `?from=/queue` from `handleStepClick`
    - Change `navigateTo(\`/parts/step/${job.stepId}?from=/queue\`)` to `navigateTo(\`/parts/step/${job.stepId}\`)`
    - _Requirements: 6.5_

  - [ ] 6.4 Update `app/pages/jobs/[id].vue` — remove `?from=` from step click navigation
    - Change `navigateTo(\`/parts/step/...?from=...\`)` to `navigateTo(\`/parts/step/${encodeURIComponent(payload.stepId)}\`)`
    - _Requirements: 6.5_

  - [ ] 6.5 Update `app/components/ProcessAdvancementPanel.vue` — inline part detail URL and remove `partDetailLink` import
...
- [ ] 7. Delete legacy navigation utilities
  - [ ] 7.1 Delete `app/utils/eyeIconLink.ts`
    - _Requirements: 6.3_

  - [ ] 7.2 Delete `app/utils/resolveBackNavigation.ts`
    - _Requirements: 6.2_

  - [ ] 7.3 Delete `tests/unit/utils/resolveBackNavigation.test.ts`
    - Remove the test file for the deleted utility
    - _Requirements: 6.2_

- [ ] 8. Deep navigation chain validation
  - [ ] 8.1 Write unit tests for deep navigation chain scenarios
