...
- [ ] 1. Type changes and helper functions
  - [ ] 1.1 Add `goalQuantity?: number` and `completedCount?: number` to `WorkQueueJob` in `server/types/computed.ts`
    - Add both optional fields after `jobPriority`
    - _Requirements: 4.1, 4.2_

  - [ ] 1.2 Remove `operatorId` from `WorkQueueResponse` in `server/types/computed.ts`
    - Remove the `operatorId: string` field from the interface
    - _Requirements: 9.1_

  - [ ] 1.3 Create `findFirstActiveStep` and `shouldIncludeStep` utility functions in `server/utils/workQueueHelpers.ts`
...
- [ ] 2. Backend: Update API route endpoints with first-step logic
  ...
  - [ ] 2.3 Write property test: First-Step Visibility (CP-WQ-FS1)
  ...
  - [ ] 2.4 Write property test: First-Step Disappearance (CP-WQ-FS2)
  ...
  - [ ] 2.5 Write property test: Backward Compatibility (CP-WQ-FS3)
  ...
  - [ ] 2.6 Write property test: Soft-Delete Respect (CP-WQ-FS6)
...
- [ ] 4. Dead endpoint removal and composable migration
  - [ ] 4.1 Delete `server/api/operator/queue/[userId].get.ts`
    - _Requirements: 7.1_

  - [ ] 4.2 Delete `app/composables/useWorkQueue.ts`
    - _Requirements: 8.1_

  - [ ] 4.3 Create `app/composables/useAdvanceBatch.ts`
  ...
  - [ ] 4.4 Update `app/pages/parts/step/[stepId].vue` to use `useAdvanceBatch`
...
- [ ] 5. Frontend: Display first-step progress
  - [ ] 5.1 Update `app/pages/queue.vue` to show progress on first-step entries
  ...
  - [ ] 5.2 Update `app/components/WorkQueueList.vue` to show progress on first-step entries
...
- [ ] 6. Align property tests with new endpoint behavior
  - [ ] 6.1 Update `tests/properties/allWorkEndpoint.property.test.ts`
  ...
  - [ ] 6.2 Update `tests/properties/workQueueAggregation.property.test.ts`
  ...
  - [ ] 6.3 Update `tests/properties/assigneeGrouping.property.test.ts`
...
- [ ] 8. Content documentation updates
  - [ ] 8.1 Delete `content/api-docs/operator/queue-user.md`
    - _Requirements: 7.2_

  - [ ] 8.2 Update `content/api-docs/operator/index.md`
    - Remove the "User queue" entry from the endpoint table
    - _Requirements: 7.3_

  - [ ] 8.3 Update `content/api-docs/operator/queue-all.md`
