...
import { generateId, createSequentialSnGenerator } from '../../../server/utils/idGenerator'
...
describe('createSequentialSnGenerator', () => {
  it('should generate sequential SN identifiers starting from 1', () => {
    let counter = 0
    ...
    expect(gen.next()).toBe('SN-00001')
    expect(gen.next()).toBe('SN-00002')
    expect(gen.next()).toBe('SN-00003')
  ...
  it('should generate a batch of sequential IDs', () => {
    ...
    expect(batch).toEqual(['SN-00001', 'SN-00002', 'SN-00003'])
    expect(counter).toBe(3)
  })
