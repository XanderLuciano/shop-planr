      ...
      fc.property(
        ...
        (userId, partId, jobId, pathId, stepId) => {
          ...
          const operations: { fn: () => void; expectedAction: AuditAction }[] = [
            ...
            },
            {
              fn: () =>
                auditService.recordStepOverrideCreated({
                  ...
                  pathId,
                  stepId,
                  metadata: { reason: 'fast-track' },
                }),
              expectedAction: 'step_override_created',
            },
            {
              fn: () =>
                auditService.recordStepOverrideReversed({ userId, partId, jobId, pathId, stepId }),
            ...
            {
              fn: () =>
                auditService.recordDeferredStepCompleted({ userId, partId, jobId, pathId, stepId }),
            ...
            },
            {
              fn: () =>
                auditService.recordStepWaived({
                  ...
                  pathId,
                  stepId,
                  metadata: { reason: 'waiver', approverId: userId },
                }),
            ...
            },
            {
              fn: () =>
                auditService.recordBomEdited({
                  userId,
                  metadata: { bomId: 'bom_1', changeDescription: 'edit', versionNumber: 1 },
                }),
          ...
          for (const op of operations) {
            const countBefore = repo
              .getEntries()
              .filter((e) => e.action === op.expectedAction).length
            op.fn()
            const countAfter = repo
              .getEntries()
              .filter((e) => e.action === op.expectedAction).length
