...
// Generate a non-empty array of existing steps with random fields and UNIQUE IDs
...
describe('Property 5: Idempotent Update', () => {
  it('updating with identical data produces toUpdate with all existing steps, empty toInsert and toSoftDelete', () => {
    fc.assert(
      fc.property(
        arbExistingStepsNonEmpty,
        (existingSteps: ProcessStep[]) => {
          // Derive input steps that mirror the existing steps exactly (with IDs for matching)
