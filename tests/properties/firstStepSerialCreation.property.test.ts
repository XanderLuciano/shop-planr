/**
 * Property-based tests for first step serial creation.
 *
 * Tests pure logic functions extracted from SerialCreationPanel.vue
...
// ---- Pure logic functions (extracted from SerialCreationPanel.vue / operator.vue) ----

/** Determines which panel to render based on step order. */
export function selectPanelType(stepOrder: number): 'serial-creation' | 'process-advancement' {
  return stepOrder === 0 ? 'serial-creation' : 'process-advancement'
}
...
/** Returns accumulated serials info: count, empty state, and IDs. */
export function getAccumulatedSerialsInfo(serialIds: string[]): { count: number; isEmpty: boolean; ids: string[] } {
  return { count: serialIds.length, isEmpty: serialIds.length === 0, ids: serialIds }
}
...
const arbWorkQueueJob = (overrides?: Partial<Record<string, fc.Arbitrary<any>>>): fc.Arbitrary<WorkQueueJob> =>
  fc.record({
    ...
    serialIds: overrides?.serialIds ?? fc.array(fc.string({ minLength: 3, maxLength: 20 }).filter(s => s.trim().length >= 3), { minLength: 0, maxLength: 15 }),
...
const arbSerialIds = (min = 0, max = 20): fc.Arbitrary<string[]> =>
  fc.array(
 ...
 * SerialCreationPanel; otherwise ProcessAdvancementPanel.
 *
 * **Validates: Requirements 1.1, 1.2, 1.3**
 */
describe('Property 1: Panel type selection based on step order', () => {
  it('stepOrder === 0 selects serial-creation panel', () => {
    fc.assert(
      fc.property(
        fc.constant(0),
        (stepOrder) => {
          expect(selectPanelType(stepOrder)).toBe('serial-creation')
        },
      ),
      { numRuns: 100 },
    )
    ...
    fc.assert(
      fc.property(
        ...
        (stepOrder) => {
          ...
          if (stepOrder === 0) {
            expect(result).toBe('serial-creation')
          } else {
      ...
      fc.property(
        ...
        (jobName, pathName, stepName) => {
          const job: WorkQueueJob = {
            ...
            serialIds: [], partCount: 0, isFinalStep: false,
          }
 ...
 *
 * For any serialIds array of length N, count matches N; empty state when N = 0;
 * all IDs are preserved.
 *
 * **Validates: Requirements 3.1, 3.3, 3.4, 3.5**
 */
describe('Property 3: Accumulated serials rendering and count', () => {
  it('count matches array length and isEmpty is correct', () => {
    fc.assert(
      fc.property(
        arbSerialIds(0, 30),
        (serialIds) => {
          const info = getAccumulatedSerialsInfo(serialIds)
          expect(info.count).toBe(serialIds.length)
          expect(info.isEmpty).toBe(serialIds.length === 0)
          expect(info.ids).toEqual(serialIds)
        },
      ),
      { numRuns: 100 },
    )
 ...
 *
 * For any set of serial IDs and any subset selection of size K:
...
describe('Property 5: Selection state drives advance payload and button state', () => {
  it('advance payload contains exactly the selected serial IDs', () => {
    fc.assert(
      fc.property(
        arbSerialIds(0, 20).chain(ids => {
        ...
        ([allIds, selectedSubset, note]) => {
          ...
          expect(payload.serialIds).toEqual(expectedIds)
          expect(payload.serialIds.length).toBe(selectedSet.size)
        },
      ),
      { numRuns: 100 },
    )
  ...
  it('advance button is disabled when no serials are selected', () => {
    fc.assert(
      fc.property(
        arbSerialIds(0, 20),
        (serialIds) => {
          const emptySelection = new Set<string>()
          const payload = buildAdvancePayload(serialIds, emptySelection, '')
          expect(payload.serialIds.length).toBe(0)
  ...
  it('advance button is enabled when at least one serial is selected', () => {
    fc.assert(
      fc.property(
        arbSerialIds(1, 20).chain(ids => {
          const uniqueIds = [...new Set(ids)]
          return fc.tuple(
            fc.constant(uniqueIds),
 ...
 * and the advance button is disabled when loading or no serials selected.
 *
...
describe('Property 8: Loading states disable corresponding controls', () => {
  ...
  it('advance button disabled when no serials selected', () => {
    fc.assert(
      fc.property(
        fc.boolean(),
        fc.boolean(),
  ...
  it('advance button enabled when not loading and serials are selected', () => {
    fc.assert(
      fc.property(
        fc.boolean(),
        fc.boolean(),
