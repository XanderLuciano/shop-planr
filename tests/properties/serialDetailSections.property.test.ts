 ...
 *
 * For any serial number with a terminal status, the appropriate status banner
 ...
 * matching the structure in serials/[id].vue.
 *
 ...
 */
function computeRoutingTabElements(state: SerialState): RenderElement[] {
...
const serialStateArb: fc.Arbitrary<SerialState> = fc.record({
  status: serialStatusArb,
...
describe('Property 6: Status banner placement above section cards', () => {
  it('scrap banner appears if and only if status is scrapped', () => {
    fc.assert(
      fc.property(serialStateArb, (state) => {
