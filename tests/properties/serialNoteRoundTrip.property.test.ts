 ...
 * text, whose serialIds includes the current serial ID, and whose stepId matches
 * the current step.
 *
...
describe('Property 4: Note creation round trip', () => {
  it('new note appears in notes array with matching text, serialIds, and stepId', () => {
    fc.assert(
      fc.property(
        ...
        idArb,
        idArb,
        (noteText, jobId, pathId, stepId, serialId, userId) => {
          ...
          expect(created.serialIds).toContain(serialId)
