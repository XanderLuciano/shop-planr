...
import type { Certificate, SerialNumber } from '~/server/types/domain'
...
async function onSubmit() {
  ...
  try {
    ...
    error.value = 'Please select a user before creating serials'
    return
  }

  saving.value = true
  try {
    const serials = await batchCreateSerials({
      jobId: props.jobId,
      pathId: props.pathId,
    ...
    emit('created', serials)
  } catch (e: any) {
    error.value = e?.data?.message ?? e?.message ?? 'Failed to create serials'
  } finally {
    saving.value = false
  }
}
