...
<template>
  <div v-if="!entries.length" class="text-xs text-(--ui-text-muted) py-4 text-center">
    No audit entries found.
  </div>
  <table v-else class="w-full text-xs">
    <thead>
      <tr class="text-(--ui-text-muted) border-b border-(--ui-border-muted)">
        <th class="text-left py-1 px-2 font-medium">Time</th>
        <th class="text-left py-1 px-2 font-medium">Action</th>
        <th class="text-left py-1 px-2 font-medium">User</th>
        <th class="text-left py-1 px-2 font-medium">Part</th>
        <th class="text-left py-1 px-2 font-medium">Cert</th>
        <th class="text-left py-1 px-2 font-medium">Details</th>
      </tr>
    </thead>
    <tbody>
      <tr
      ...
      >
        ...
        <td class="py-1 px-2 whitespace-nowrap">
          <span class="inline-flex items-center gap-1" :class="actionConfig[entry.action]?.color">
            <UIcon :name="actionConfig[entry.action]?.icon" class="size-3" />
        ...
        <td class="py-1 px-2 text-(--ui-text-muted)">
          <span v-if="entry.fromStepId && entry.toStepId"
            >{{ entry.fromStepId }} → {{ entry.toStepId }}</span
          >
