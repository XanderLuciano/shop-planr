  ...
  <div class="space-y-2">
    <div class="grid grid-cols-2 gap-2">
      <div>
        ...
        <UInput v-model="name" size="sm" placeholder="Full name" />
      </div>
      <div>
        <label class="block text-xs text-(--ui-text-muted) mb-0.5">Department</label>
        <UInput v-model="department" size="sm" placeholder="e.g. Machining, QC" />
      </div>
    </div>
    <div v-if="user" class="flex items-center gap-2">
      <USwitch v-model="active" size="sm" />
    ...
    <div class="flex gap-2 justify-end">
      ...
      <UButton size="xs" :label="user ? 'Save' : 'Create User'" @click="onSubmit" />
    </div>
  </div>
</template>
