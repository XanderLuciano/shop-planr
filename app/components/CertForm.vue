  ...
  <form class="space-y-2" @submit.prevent="onSubmit">
    <div class="grid grid-cols-2 gap-2">
      <div>
        ...
        <USelect v-model="certType" :items="typeOptions" size="sm" value-key="value" />
      </div>
      <div>
        <label class="block text-xs text-(--ui-text-muted) mb-0.5">Name</label>
        <UInput v-model="certName" size="sm" placeholder="Certificate name" />
      </div>
    </div>
    ...
    <div>
      ...
      <div v-if="metadataEntries.length" class="space-y-1">
        <div v-for="(entry, i) in metadataEntries" :key="i" class="flex items-center gap-1.5">
          <UInput v-model="entry.key" size="sm" placeholder="Key" class="flex-1" />
          <UInput v-model="entry.value" size="sm" placeholder="Value" class="flex-1" />
          <UButton
            icon="i-lucide-x"
            size="xs"
            variant="ghost"
    ...
    <div class="flex justify-end">
      <UButton type="submit" size="xs" label="Save Certificate" :disabled="!certName.trim()" />
    </div>
  </form>
</template>
