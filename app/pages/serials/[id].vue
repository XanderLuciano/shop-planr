...
import type { WorkQueueJob, EnrichedSerial, SnStepStatusView } from '~/server/types/computed'
import type { StepNote, CertAttachment, SnStepOverride } from '~/server/types/domain'

const route = useRoute()
const serialId = route.params.id as string
...
// Lifecycle data
const stepStatuses = ref<SnStepStatusView[]>([])
const overrides = ref<SnStepOverride[]>([])
...
const sortedSiblings = computed(() => {
  const list = [...siblingSerials.value] as EnrichedSerial[]
...
const siblingTotalCount = computed(() => siblingSerials.value.length)
const siblingCompletedCount = computed(() =>
  siblingSerials.value.filter((s: any) => s.status === 'completed' || s.currentStepIndex === -1).length,
)
const siblingInProgressCount = computed(() =>
  siblingSerials.value.filter((s: any) => s.status === 'in-progress' || (s.currentStepIndex >= 0 && s.status !== 'scrapped')).length,
)

watch(activeTab, async (tab) => {
  if (tab === 'serials' && !siblingsLoaded.value) {
    await fetchSiblings()
...
// Computed states
const isScrapped = computed(() => serial.value?.status === 'scrapped')
const isCompleted = computed(() => serial.value?.status === 'completed' || serial.value?.currentStepIndex === -1)
const isForceCompleted = computed(() => serial.value?.forceCompleted === true)
const isInProgress = computed(() => serial.value?.status === 'in_progress' || serial.value?.status === 'in-progress' || (!isScrapped.value && !isCompleted.value && serial.value?.currentStepIndex !== undefined && serial.value.currentStepIndex >= 0))

const currentStep = computed(() => {
  if (!path.value || !serial.value || serial.value.currentStepIndex < 0) return null
  return path.value.steps[serial.value.currentStepIndex] ?? null
})
...
const workQueueJob = computed<WorkQueueJob | null>(() => {
  if (!serial.value || !job.value || !path.value || !currentStep.value) return null
  const steps = path.value.steps
  const stepIndex = serial.value.currentStepIndex
...
function getStepStatusForStep(stepId: string): SnStepStatusView | undefined {
...
async function handleAdvance(payload: { serialIds: string[], note?: string }) {
  if (!serial.value || !currentStep.value) return
  const { advanceSerial } = useSerials()
  const { operatorId } = useOperatorIdentity()

  advanceLoading.value = true
  try {
    for (const sid of payload.serialIds) {
      await advanceSerial(sid, operatorId.value ?? 'system')
    }
    ...
    toast.add({ title: 'Part advanced', description: `${serial.value!.id} moved forward`, color: 'success' })
  } catch (e: any) {
...
async function loadLifecycleData() {
  try {
    stepStatuses.value = await getStepStatuses(serialId)
  } catch { stepStatuses.value = [] }

  try {
    overrides.value = await $fetch<SnStepOverride[]>(`/api/serials/${encodeURIComponent(serialId)}/overrides`)
  } catch { overrides.value = [] }

  try {
    certAttachments.value = await $fetch<CertAttachment[]>(`/api/serials/${encodeURIComponent(serialId)}/cert-attachments`)
...
<template>
  <div class="p-4 space-y-4 max-w-5xl">
    ...
    <!-- Error -->
    <div v-else-if="error && !serial" class="text-sm text-red-500 py-8">{{ error }}</div>

    <template v-else-if="serial && job && path">
      <!-- Header -->
      <div class="space-y-1">
        <div class="flex items-center gap-2">
          <h1 class="text-lg font-bold text-(--ui-text-highlighted)">{{ serial.id }}</h1>
          <UBadge
      ...
      <div v-if="isScrapped" class="flex items-center gap-2 px-3 py-2 rounded-md bg-red-500/10 text-red-600 text-sm">
        ...
        <div>
          ...
          <span v-if="serial.scrapReason"> — {{ serial.scrapReason.replace(/_/g, ' ') }}</span>
          <span v-if="serial.scrapExplanation">: {{ serial.scrapExplanation }}</span>
          <div v-if="serial.scrappedAt" class="text-xs mt-0.5">
            {{ serial.scrappedBy ? `By ${serial.scrappedBy}` : '' }} on {{ new Date(serial.scrappedAt).toLocaleString() }}
          </div>
        </div>
      </div>
      ...
      <div v-if="isForceCompleted && !isScrapped" class="flex items-center gap-2 px-3 py-2 rounded-md bg-amber-500/10 text-amber-600 text-sm">
        ...
        <div>
          ...
          <span v-if="serial.forceCompletedReason"> — {{ serial.forceCompletedReason }}</span>
          <div v-if="serial.forceCompletedAt" class="text-xs mt-0.5">
            {{ serial.forceCompletedBy ? `By ${serial.forceCompletedBy}` : '' }} on {{ new Date(serial.forceCompletedAt).toLocaleString() }}
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'routing'" class="space-y-4">
        ...
        <SectionCard title="Routing" icon="i-lucide-route">
          ...
          <div class="border border-(--ui-border) rounded-md overflow-hidden divide-y divide-(--ui-border)">
            <div
              ...
              :class="{
                'bg-(--ui-primary)/10 border-l-2 border-l-(--ui-primary)': serial.currentStepIndex === index,
                'bg-(--ui-bg-elevated)/30': serial.currentStepIndex !== index,
              }"
            ...
            >
              ...
              <div class="flex items-center justify-center size-6 rounded-full text-xs font-bold shrink-0"
                :class="index < serial.currentStepIndex || isCompleted
                  ? 'bg-green-500/20 text-green-600'
                  : serial.currentStepIndex === index
                    ? 'bg-(--ui-primary)/20 text-(--ui-primary)'
                    : 'bg-(--ui-bg-elevated) text-(--ui-text-muted)'"
              >
                <UIcon v-if="index < serial.currentStepIndex || isCompleted" name="i-lucide-check" class="size-3.5" />
              ...
              <div class="shrink-0">
                <UBadge v-if="getStepDistribution(step.id)" variant="subtle" color="neutral" size="xs">
                  {{ getStepDistribution(step.id)!.serialCount }} parts
                </UBadge>
              </div>
            </div>
          </div>
        ...
        <SectionCard title="Notes" icon="i-lucide-message-square">
          <div class="space-y-3">
            <PartDetailNotes :serial-id="serialId" hide-heading />

            <!-- Add Note button — visible only for in-progress serials when form is closed -->
            <UButton
      ...
      <!-- Serials Tab -->
      <div v-if="activeTab === 'serials'" class="space-y-3">
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-(--ui-border)">
              <tr
                ...
                :class="sib.id === serialId ? 'bg-(--ui-primary)/10 font-medium' : 'hover:bg-(--ui-bg-elevated)/50'"
                @click="navigateTo(`/serials/${encodeURIComponent(sib.id)}`)"
              >
              ...
              <tr v-if="sortedSiblings.length === 0">
                <td colspan="4" class="px-3 py-6 text-center text-(--ui-text-muted)">No sibling serials found.</td>
              </tr>
            </tbody>
          </table>
        </div>
