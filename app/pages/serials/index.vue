...
const jobOptions = computed(() => {
  const names = [...new Set(serials.value.map(s => s.jobName))].sort()
  return [{ label: 'All Jobs', value: '__all__' }, ...names.map(n => ({ label: n, value: n }))]
})
const pathOptions = computed(() => {
  const names = [...new Set(serials.value.map(s => s.pathName))].sort()
  return [{ label: 'All Paths', value: '__all__' }, ...names.map(n => ({ label: n, value: n }))]
})
const stepOptions = computed(() => {
  const names = [...new Set(serials.value.map(s => s.currentStepName))].sort()
...
const assigneeOptions = computed(() => {
  const names = [...new Set(serials.value.map(s => s.assignedTo).filter((v): v is string => !!v))].sort()
  return [
...
<template>
  <div class="p-4 space-y-3 max-w-6xl">
    ...
    <div v-if="filtersActive" class="text-xs text-(--ui-text-muted)">
      Showing {{ filteredCount }} of {{ totalCount }} serials
    </div>
    ...
    <div v-else-if="error" class="flex items-center gap-2 text-xs text-(--ui-error)">
      <span>{{ error }}</span>
      <UButton size="xs" variant="ghost" icon="i-lucide-refresh-cw" label="Retry" @click="fetchSerials" />
    </div>
