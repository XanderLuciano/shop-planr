...
import { SQLiteSerialRepository } from '../repositories/sqlite/serialRepository'
...
function createServices(db: Database.Database) {
  ...
  return { jobService, pathService, serialService, certService, templateService, noteService, userService, libraryService }
}
...
export function seedDatabase(dbPath: string): void {
  ...
  try {
    ...
    const certHardAnodize = svc.certService.createCert({
      type: 'process',
    ...
    const j1sns = svc.serialService.batchCreateSerials(
      { jobId: job1.id, pathId: path1.id, quantity: 20, certId: certAluminum6061.id },
      mike.id
    )
    // Advance 16 past CNC Machine (step 0 → 1)
    for (let i = 0; i < 16; i++) svc.serialService.advanceSerial(j1sns[i].id, mike.id)
    // Advance 12 past Deburr (step 1 → 2)
    for (let i = 0; i < 12; i++) svc.serialService.advanceSerial(j1sns[i].id, lisa.id)
    // Advance 10 past Inspection (step 2 → 3)
    for (let i = 0; i < 10; i++) svc.serialService.advanceSerial(j1sns[i].id, sarah.id)
    // Advance 8 past Chemfilm (step 3 → 4)
    for (let i = 0; i < 8; i++) svc.serialService.advanceSerial(j1sns[i].id, sarah.id)
    // Complete 5 (past Final Inspection)
    for (let i = 0; i < 5; i++) svc.serialService.advanceSerial(j1sns[i].id, sarah.id)
    // Attach chemfilm cert to the 8 that went through chemfilm
    for (let i = 0; i < 8; i++) {
      svc.certService.attachCertToSerial({
        certId: certChemfilm.id, serialId: j1sns[i].id,
    ...
    svc.noteService.createNote({
      ...
      serialIds: [j1sns[13].id, j1sns[14].id],
      text: 'Sharp edge on pocket corner — needs extra deburr pass',
      userId: lisa.id
    })
    console.log(`  Job: ${job1.name} — 20 SNs (5 done, 3 at Final Insp, 2 at Chemfilm, 2 at Insp, 4 at Deburr, 4 at CNC)`)
    ...
    const j2sns = svc.serialService.batchCreateSerials(
    ...
    for (let i = 0; i < 8; i++) svc.serialService.advanceSerial(j2sns[i].id, tony.id)
    // Advance 6 past Stress Relief (step 1 → 2)
    for (let i = 0; i < 6; i++) svc.serialService.advanceSerial(j2sns[i].id, tony.id)
    // Advance 4 past second CNC (step 2 → 3)
    for (let i = 0; i < 4; i++) svc.serialService.advanceSerial(j2sns[i].id, tony.id)
    // Advance 3 past Deburr (step 3 → 4)
    for (let i = 0; i < 3; i++) svc.serialService.advanceSerial(j2sns[i].id, lisa.id)
    // Advance 2 past Inspection (step 4 → 5)
    for (let i = 0; i < 2; i++) svc.serialService.advanceSerial(j2sns[i].id, sarah.id)
    // Note about stress relief vendor delay
    svc.noteService.createNote({
      jobId: job2.id, pathId: path2.id, stepId: path2.steps[1].id,
      serialIds: [j2sns[6].id, j2sns[7].id],
      text: 'Vendor batch delayed — expected back Thursday',
      userId: tony.id
    })
    console.log(`  Job: ${job2.name} — 10 SNs (0 done, 2 at Hard Anodize, 1 at Insp, 1 at Deburr, 2 at CNC2, 2 at Stress Relief, 2 at CNC1)`)
    ...
    const j3aSNs = svc.serialService.batchCreateSerials(
    ...
    for (let i = 0; i < 20; i++) svc.serialService.advanceSerial(j3aSNs[i].id, mike.id)
    // Advance 15 past Heat Treat (step 1 → 2)
    for (let i = 0; i < 15; i++) svc.serialService.advanceSerial(j3aSNs[i].id, mike.id)
    // Attach heat treat cert to the 15 that went through
    for (let i = 0; i < 15; i++) {
      svc.certService.attachCertToSerial({
        certId: certHeatTreat.id, serialId: j3aSNs[i].id,
        stepId: path3a.steps[1].id, userId: mike.id,
        jobId: job3.id, pathId: path3a.id
      })
    }
    // Advance 10 past second CNC (step 2 → 3)
    for (let i = 0; i < 10; i++) svc.serialService.advanceSerial(j3aSNs[i].id, mike.id)
    // Advance 8 past Deburr (step 3 → 4)
    for (let i = 0; i < 8; i++) svc.serialService.advanceSerial(j3aSNs[i].id, lisa.id)
    // Advance 6 past Inspection (step 4 → 5)
    for (let i = 0; i < 6; i++) svc.serialService.advanceSerial(j3aSNs[i].id, sarah.id)
    // Advance 4 past Passivate (step 5 → 6)
    for (let i = 0; i < 4; i++) svc.serialService.advanceSerial(j3aSNs[i].id, sarah.id)
    // Attach passivation cert
    for (let i = 0; i < 4; i++) {
      svc.certService.attachCertToSerial({
        certId: certPassivate.id, serialId: j3aSNs[i].id,
        stepId: path3a.steps[5].id, userId: sarah.id,
        jobId: job3.id, pathId: path3a.id
      })
    }
    // Complete 2
    for (let i = 0; i < 2; i++) svc.serialService.advanceSerial(j3aSNs[i].id, sarah.id)
    ...
    const j3bSNs = svc.serialService.batchCreateSerials(
      { jobId: job3.id, pathId: path3b.id, quantity: 15, certId: certSteel4140.id },
      tony.id
    )
    // Steps: 0=CNC, 1=Deburr, 2=Inspection
    for (let i = 0; i < 12; i++) svc.serialService.advanceSerial(j3bSNs[i].id, tony.id)
    for (let i = 0; i < 8; i++) svc.serialService.advanceSerial(j3bSNs[i].id, lisa.id)
    for (let i = 0; i < 5; i++) svc.serialService.advanceSerial(j3bSNs[i].id, sarah.id)

    svc.noteService.createNote({
      jobId: job3.id, pathId: path3a.id, stepId: path3a.steps[4].id,
      serialIds: [j3aSNs[7].id],
      text: 'OD dimension at high end of tolerance — flagged for review',
      userId: sarah.id
    })
    console.log(`  Job: ${job3.name} — 40 SNs across 2 paths (7 completed total)`)
    ...
    const j4sns = svc.serialService.batchCreateSerials(
    ...
    // All 15 past CNC
    for (let i = 0; i < 15; i++) svc.serialService.advanceSerial(j4sns[i].id, mike.id)
    // 14 past Deburr
    for (let i = 0; i < 14; i++) svc.serialService.advanceSerial(j4sns[i].id, lisa.id)
    // 13 past Bead Blast
    for (let i = 0; i < 13; i++) svc.serialService.advanceSerial(j4sns[i].id, lisa.id)
    // 12 past Inspection
    for (let i = 0; i < 12; i++) svc.serialService.advanceSerial(j4sns[i].id, sarah.id)
    // 10 past Anodize
    for (let i = 0; i < 10; i++) svc.serialService.advanceSerial(j4sns[i].id, sarah.id)
    // Attach anodize cert
    for (let i = 0; i < 10; i++) {
      svc.certService.attachCertToSerial({
        certId: certAnodize.id, serialId: j4sns[i].id,
        stepId: path4.steps[4].id, userId: sarah.id,
        jobId: job4.id, pathId: path4.id
      })
    }
    // Complete 8
    for (let i = 0; i < 8; i++) svc.serialService.advanceSerial(j4sns[i].id, sarah.id)
    console.log(`  Job: ${job4.name} — 15 SNs (8 done, 2 at Final Insp, 2 at Anodize, 1 at Insp, 1 at Bead Blast, 1 at Deburr)`)
    ...
    const j5sns = svc.serialService.batchCreateSerials(
    ...
    // 20 past CNC
    for (let i = 0; i < 20; i++) svc.serialService.advanceSerial(j5sns[i].id, tony.id)
    // 12 past Deburr
    for (let i = 0; i < 12; i++) svc.serialService.advanceSerial(j5sns[i].id, lisa.id)
    // 5 past Inspection
    for (let i = 0; i < 5; i++) svc.serialService.advanceSerial(j5sns[i].id, sarah.id)

    svc.noteService.createNote({
      jobId: job5.id, pathId: path5.id, stepId: path5.steps[0].id,
      serialIds: [j5sns[22].id, j5sns[23].id, j5sns[24].id],
      text: 'Tool wear detected on lathe — replaced insert, re-verify dimensions on these parts',
      userId: tony.id
    })
    console.log(`  Job: ${job5.name} — 30 SNs (0 done, 5 at Passivate, 7 at Insp, 8 at Deburr, 10 at CNC)`)
...
export function resetDatabase(dbPath: string): void {
  ...
  try {
    ...
    if (sampleJobIds.length > 0) {
      ...
      // Find SAMPLE- serial IDs (for cert_attachments cleanup)
      const sampleSerialIds = (db.prepare(`SELECT id FROM serials WHERE job_id IN (${jobPlaceholders})`).all(...sampleJobIds) as { id: string }[])
        .map(r => r.id)
      ...
      db.transaction(() => {
        ...
        // Delete cert_attachments, sn_step_statuses, sn_step_overrides for SAMPLE serials
        if (sampleSerialIds.length > 0) {
          const serialPlaceholders = sampleSerialIds.map(() => '?').join(',')
          db.prepare(`DELETE FROM cert_attachments WHERE serial_id IN (${serialPlaceholders})`).run(...sampleSerialIds)
          db.prepare(`DELETE FROM sn_step_statuses WHERE serial_id IN (${serialPlaceholders})`).run(...sampleSerialIds)
          db.prepare(`DELETE FROM sn_step_overrides WHERE serial_id IN (${serialPlaceholders})`).run(...sampleSerialIds)
        }
        ...
        // Delete serials for SAMPLE jobs
        db.prepare(`DELETE FROM serials WHERE job_id IN (${jobPlaceholders})`).run(...sampleJobIds)
      ...
      console.log(`  Deleted ${sampleJobIds.length} jobs, ${sampleSerialIds.length} serials, ${samplePathIds.length} paths`)
    } else {
