...
function hasSampleData(db: Database.Database): boolean {
  const row = db.prepare("SELECT COUNT(*) as cnt FROM jobs WHERE name LIKE 'SAMPLE-%'").get() as {
    cnt: number
  }
  return row.cnt > 0
}
...
export function seedDatabase(dbPath: string): void {
  ...
  try {
    ...
    const locations = [
      ...
      'Deburr Station',
      'Vendor - Anodize Co.',
      'Vendor - HeatTreat Inc.',
      'Vendor - Chemfilm Services',
      'Shipping',
      'Laser Room',
    ]
    ...
    const lisa = svc.userService.createUser({
      name: 'SAMPLE-Lisa',
      department: 'Deburr / Finishing',
    })
    ...
    const certAluminum6061 = svc.certService.createCert({
      ...
      metadata: {
        grade: '6061-T6',
        supplier: 'AlumSupply Inc.',
        lotNumber: 'L-2025-0447',
        spec: 'AMS-QQ-A-250/11',
      },
    })
    const certAluminum7075 = svc.certService.createCert({
      ...
      metadata: {
        grade: '7075-T651',
        supplier: 'AlumSupply Inc.',
        lotNumber: 'L-2025-0892',
        spec: 'AMS-QQ-A-250/12',
      },
    })
    ...
    const certChemfilm = svc.certService.createCert({
      ...
      metadata: {
        vendor: 'Chemfilm Services',
        process: 'Chromate Conversion',
        spec: 'MIL-DTL-5541 Class 1A',
      },
    })
    const certPassivate = svc.certService.createCert({
      ...
      metadata: {
        vendor: 'Chemfilm Services',
        process: 'Citric Acid Passivation',
    ...
    for (let i = 0; i < 8; i++) {
      svc.certService.attachCertToSerial({
        certId: certChemfilm.id,
        serialId: j1parts[i].id,
        stepId: path1.steps[3].id,
        userId: sarah.id,
        jobId: job1.id,
        pathId: path1.id,
      })
    ...
    svc.noteService.createNote({
      jobId: job1.id,
      pathId: path1.id,
      stepId: path1.steps[1].id,
      partIds: [j1parts[13].id, j1parts[14].id],
      text: 'Sharp edge on pocket corner — needs extra deburr pass',
      userId: lisa.id,
    })
    console.log(
      `  Job: ${job1.name} — 20 parts (5 done, 3 at Final Insp, 2 at Chemfilm, 2 at Insp, 4 at Deburr, 4 at CNC)`
    )
    ...
    svc.noteService.createNote({
      jobId: job2.id,
      pathId: path2.id,
      stepId: path2.steps[1].id,
      partIds: [j2parts[6].id, j2parts[7].id],
      text: 'Vendor batch delayed — expected back Thursday',
      userId: tony.id,
    })
    console.log(
      `  Job: ${job2.name} — 10 parts (0 done, 2 at Hard Anodize, 1 at Insp, 1 at Deburr, 2 at CNC2, 2 at Stress Relief, 2 at CNC1)`
    )
    ...
    for (let i = 0; i < 15; i++) {
      svc.certService.attachCertToSerial({
        certId: certHeatTreat.id,
        serialId: j3aParts[i].id,
        stepId: path3a.steps[1].id,
        userId: mike.id,
        jobId: job3.id,
        pathId: path3a.id,
      })
    ...
    for (let i = 0; i < 4; i++) {
      svc.certService.attachCertToSerial({
        certId: certPassivate.id,
        serialId: j3aParts[i].id,
        stepId: path3a.steps[5].id,
        userId: sarah.id,
        jobId: job3.id,
        pathId: path3a.id,
      })
    ...
    for (let i = 0; i < 10; i++) {
      svc.certService.attachCertToSerial({
        certId: certAnodize.id,
        serialId: j4parts[i].id,
        stepId: path4.steps[4].id,
        userId: sarah.id,
        jobId: job4.id,
        pathId: path4.id,
      })
    ...
    console.log(
      `  Job: ${job4.name} — 15 parts (8 done, 2 at Final Insp, 2 at Anodize, 1 at Insp, 1 at Bead Blast, 1 at Deburr)`
    )
    ...
    console.log(
      `  Job: ${job5.name} — 30 parts (0 done, 5 at Passivate, 7 at Insp, 8 at Deburr, 10 at CNC)`
    )
...
export function resetDatabase(dbPath: string): void {
  ...
  try {
    ...
    const sampleJobIds = (
      db.prepare("SELECT id FROM jobs WHERE name LIKE 'SAMPLE-%'").all() as { id: string }[]
    ).map((r) => r.id)
    ...
    if (sampleJobIds.length > 0) {
      ...
      const samplePartIds = (
        db
          .prepare(`SELECT id FROM parts WHERE job_id IN (${jobPlaceholders})`)
          .all(...sampleJobIds) as { id: string }[]
      ).map((r) => r.id)

      // Find SAMPLE- path IDs (for process_steps cleanup)
      const samplePathIds = (
        db
          .prepare(`SELECT id FROM paths WHERE job_id IN (${jobPlaceholders})`)
          .all(...sampleJobIds) as { id: string }[]
      ).map((r) => r.id)

      db.transaction(() => {
        // Delete step_notes for SAMPLE jobs
        db.prepare(`DELETE FROM step_notes WHERE job_id IN (${jobPlaceholders})`).run(
          ...sampleJobIds
        )
        ...
        if (samplePartIds.length > 0) {
          ...
          db.prepare(`DELETE FROM cert_attachments WHERE part_id IN (${partPlaceholders})`).run(
            ...samplePartIds
          )
          db.prepare(`DELETE FROM part_step_statuses WHERE part_id IN (${partPlaceholders})`).run(
            ...samplePartIds
          )
          db.prepare(`DELETE FROM part_step_overrides WHERE part_id IN (${partPlaceholders})`).run(
            ...samplePartIds
          )
        }

        // Delete audit_entries for SAMPLE jobs
        db.prepare(`DELETE FROM audit_entries WHERE job_id IN (${jobPlaceholders})`).run(
          ...sampleJobIds
        )
        ...
        if (samplePathIds.length > 0) {
          ...
          db.prepare(`DELETE FROM process_steps WHERE path_id IN (${pathPlaceholders})`).run(
            ...samplePathIds
          )
        }
        ...
        db.prepare(`DELETE FROM bom_contributing_jobs WHERE job_id IN (${jobPlaceholders})`).run(
          ...sampleJobIds
        )
      ...
      console.log(
        `  Deleted ${sampleJobIds.length} jobs, ${samplePartIds.length} parts, ${samplePathIds.length} paths`
      )
    } else {
